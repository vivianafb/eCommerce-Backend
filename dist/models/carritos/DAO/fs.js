...
class CarritoFSDAO {
  ...
  async leer(archivo) {
    this.carrito = JSON.parse(await _fs.default.promises.readFile(archivo, 'utf-8'));
  }

  async guardar() {
    await _fs.default.promises.writeFile(this.nombreArchivo, JSON.stringify(this.carrito, null, '\t'));
  }
  ...
  async query(options) {
    await this.leer(this.nombreArchivo); // type Conditions = (aProduct) => boolean;

    const query = [];
    if (options.nombre) query.push(aCarrito => aCarrito.nombre == options.nombre);
    if (options.precio) query.push(aProduct => aCarrito.precio == options.precio);
