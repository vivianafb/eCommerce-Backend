...
var _carrito = require("../apis/carrito");
...
class Carrito {
  async validacion(req, res, next) {
    ...
    if (!createdAt || !producto_id) return res.status(400).json({
  ...
  async checkCarritoExists(req, res, next) {
    ...
    const carrito = await _carrito.carritoAPI.getCarrito(tableName, id);
  ...
  async getCarrito(req, res) {
    ...
    if (id) {
      const result = await _carrito.carritoAPI.getCarrito(id);
      console.log(result);
    ...
    res.json({
      data: await _carrito.carritoAPI.getCarrito()
    });
  }

  async addCarrito(req, res) {
    const newItem = await _carrito.carritoAPI.addCarrito(req.body);
    res.json({
  ...
  async deleteCarrito(req, res) {
    const id = req.params.id;
    await _carrito.carritoAPI.deleteCarrito(id);
    res.json({
      msg: 'carrito borrado'
    });
  }
